...
import type { param_dfn_reducer_T } from './param_dfn_reducer_T.js'
...
export function param_value_r_param_name_(
	...
	const param_name_r_flag = flag_r_param_name_(param_name_r_param_dfn)
	let i = 0
	while (i < arg_a.length) {
		...
		if (match) {
			...
			const param_ = ((
				param_name_r_param_dfn
				&& isArray(param_name_r_param_dfn[param_name])
				&& param_name_r_param_dfn[param_name][1]
